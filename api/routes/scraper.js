  ...
  try {
    ...
    const title = TitleMatches
      ? TitleMatches.map((match) =>
          match.replace(/<\/?span[^>]*>|<\/?a[^>]*>/g, "")
        )
      : [];

    const descriptions = await page.$$eval("div div div div div", (div) =>
      div.map((div) => div.textContent)
    );
    ...
    const images = imageMatches
      ? imageMatches
          ...
          .filter((src) => src.includes("w=3840&amp"))
      : null;
    ...
    const hrefs = HrefMatches
      ? HrefMatches.map((match) => match.match(/<a[^>]*href="([^"]*)"/)[1])
      : null;
